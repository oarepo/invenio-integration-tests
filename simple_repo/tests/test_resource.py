...
from simple_repo.model import ModelResourceConfig, current_service
...
BASE_URL = ModelResourceConfig.url_prefix
"""
...
def test_read(client_with_credentials, sample_record, search_clear, app):
    ...
    if is_action_allowed("read", True):
        assert get_response.json["title"] == sample_record["title"]
...
def test_create(
    ...
    if is_action_allowed("create", True):
        for sample_metadata_point, created_response in zip(
            ...
            assert (
                created_response_reread.json["title"]
                == sample_metadata_point["title"]
            )
...
def test_update(
    ...
    if is_action_allowed("update", True):
        assert old_record_read_response_json["title"] == sample_record['title']
        assert (
            update_response.json["title"]
            == sample_metadata_list[2]["title"]
            != old_record_read_response_json["title"]
        )
        assert (
            updated_record_read_response.json["title"]
            == sample_metadata_list[2]["title"]
        )
