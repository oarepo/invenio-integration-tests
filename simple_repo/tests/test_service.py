...
def test_create(app, db, record_service, sample_metadata_list, search_clear):
    ...
    for sample_metadata_point, created_record in zip(
        ...
        assert (
            created_record_reread.data["title"] == sample_metadata_point["title"]
        )
...
def test_update(
    ...
    assert old_record_data["title"] == sample_record["title"]
    assert (
        updated_record.data["title"]
        == sample_metadata_list[2]["title"]
        != old_record_data["title"]
    )
